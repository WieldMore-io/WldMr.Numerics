...
module DiffOps =
    ...
    let jacobian f =
        let fj = jacobianT f
        fun x -> fj x |> GenMat.ofArray2D |> Lite.Backend.Transpose_M
    ...
    let jacobian' f =
        let fj = jacobianT' f
        fun x -> fj x |> fun (r, j) -> (r, j |> GenMat.ofArray2D |> Lite.Backend.Transpose_M)
    ...
    let inline div' f x =
        ...
        v, Lite.Backend.Sum_V(Lite.Backend.Diagonal_M(j |> GenMat.ofArray2D))

    /// Divergence of a vector-to-vector function `f`, at point `x`. Defined only for functions with a square Jacobian matrix.
    let inline div f x =
        let j = jacobianT f x
        if Array2D.length1 j <> Array2D.length2 j then ErrorMessages.InvalidArgDiv()
        Lite.Backend.Sum_V(Lite.Backend.Diagonal_M(j |> GenMat.ofArray2D))
